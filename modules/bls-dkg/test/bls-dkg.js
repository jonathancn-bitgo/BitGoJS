...
describe('bls-dkg', function() {
  ...
  before('generates user polynomials and shares', function() {
    ...
    userSecretShares = secretShares([userSubKey.parentSk, userPolynomial[1]], n);
    userPublicShare = publicShare([userSubKey.parentSk]);
  });
  ...
  it('verifies user-backup subkey sig with public derived', async function() {
    ...
    const derivedUserPublicShare = publicShare([derivedUserSk.parentSk]);
    ...
    const userSecretShares = secretShares([derivedUserSk.parentSk, userPolynomial[1]], n);
